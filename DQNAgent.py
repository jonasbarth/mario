...
class DQNAgent:
    ...
    def __init__(self, batch_size, gamma, eps_start, eps_end, eps_decay, target_update, num_episodes, screen_height, screen_width):
        self.n_actions = 8
    ...
    def train(self):
        for i_episode in range(self.num_episodes):
            ...
            state = self.client.init_env()[0]
            state = self.add_dims(state)
            ...
            for t in count():
                ...
                print("Taking action")
                next_state, reward, alive = self.client.step(action)
                print("Took action", action, "and got reward", reward)

                next_state = self.add_dims(next_state)
    ...
    def add_dims(self, state):
        rand_tensor = torch.randn((1,3,240,256))
    
        state_tensor = torch.FloatTensor(state)
        rand_tensor[0][0] = state_tensor
        rand_tensor[0][1] = state_tensor
        rand_tensor[0][2] = state_tensor
